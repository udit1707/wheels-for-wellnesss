...
def fetchStatus(patient_severity,ambulance_distance,ambulance_time_pt_hp):
...
# value : ambulance id
    ...
    for key, value in ans_dic.items():
        hp_time = ambulance_time_pt_hp.get(key)
        if hp_time:
            mino = sys.maxsize
            hp_id = 0
            for k, v in hp_time.items():
                if mino>v:
                    mino = v
                    hp_id = k
            li = []
